...
### 2.4 CSS scoping via `data-block-id`

- Root `<div>` receives `data-block-id={blockId}` from `appBridge.context('blockId').get()`.
- All global CSS selectors in `style.css` prefixed with `[data-block-id]` to prevent style leakage between block instances.
